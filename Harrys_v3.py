...
def temp(row):
    if ((df["user_id"] == row.user_id) & (df[
                                              "plan_end_type"] == "on_going")).any():  # where .any() is "if any of the statements are true, this is true". Need to understand this better still (can also be witten as sum() > 0
...
# df['testys'] = round((((df.groupby(['user_id'])['plan_end_date'].transform('max')) - df[
